...
[キーマップ](ja/keymap) を定義するときは、それぞれのキーに有効な定義が必要です。このページは、QMK で使えるキーコードに相当するシンボルについて記述しています。

このページは参照のみです。それぞれのキーの種類毎のリンク先のページに、それぞれのキーの機能についてもっと詳細に記載しています。

## 基本的なキーコード :id=basic-keycodes

[基本的なキーコード](ja/keycodes_basic) も見てください。
...
[オーディオ](ja/feature_audio) も見てください。
...
[バックライト](ja/feature_backlight) も見てください。
...
[ブートマジック](ja/feature_bootmagic) も見てください。
...
[Bluetooth](ja/feature_bluetooth) も見てください。
...
[動的マクロ](ja/feature_dynamic_macros) も見てください。 
...
[グレイブエスケープ](ja/feature_grave_esc) も見てください。
...
[キーロック](ja/feature_key_lock) も見てください。
...
|`OSL(layer)`    |次のキーが押されるまで、一時的にレイヤーをアクティブにします。詳細は [ワンショットキー](ja/one_shot_keys) のとおり。 |
|`LM(layer, mod)`|`mod` がアクティブな状態で (MO のように) 一時的にレイヤーをアクティブにします。ここでは、`mod` は mods_bit のことです。Mod については [こちら](ja/mod_tap) で見ることができます。実装例: `LM(LAYER_1, MOD_LALT)` |
...
[リーダーキー](ja/feature_leader_key) も見てください。
...
[マウスキー](ja/feature_mouse_keys) も見てください。
...
[RGB ライト](ja/feature_rgblight) も見てください。
...
[RGB マトリックスライト](ja/feature_rgb_matrix) も見てください。
...
[感熱式プリンタ](ja/feature_thermal_printer) も見てください。
...
[US ANSI シフト済シンボル](ja/keycodes_us_ansi_shifted) も見てください。
...
[ワンショットキー](ja/one_shot_keys) も見てください。
...
[Space Cadet](ja/feature_space_cadet) も見てください。
...
[スワップハンド](ja/feature_swap_hands) も見てください。
...
[ユニコードサポート](ja/feature_unicode) も見てください。
